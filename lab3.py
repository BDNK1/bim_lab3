...
def move_handle(build_ele, handle_prop, input_pnt, doc):
    ...
    if handle_prop.handle_id == "BeamHeight":
        ...
        hole_height = (
            build_ele.BeamHeight.value - build_ele.TopShHeight.value - RADIUS
        )
        ...
        if (build_ele.RibThick.value <= build_ele.VaryingRibThick.value):
            build_ele.VaryingRibThick.value = build_ele.RibThick.value - DIFF
        elif (
...
def modify_element_property(build_ele, name, value):
    if name == "BeamHeight":
        ...
        if change < 0:
            change = abs(change)
            if build_ele.TopShHeight.value > TOPHEIGHT:
                if build_ele.TopShHeight.value - change < TOPHEIGHT:
                    change -= build_ele.TopShHeight.value - TOPHEIGHT
                    build_ele.TopShHeight.value = TOPHEIGHT
                else:
                    build_ele.TopShHeight.value -= change
                    change = 0.0
            if (change != 0) and (build_ele.BotShUpHeight.value > BOTUPHEIGHT):
                if build_ele.BotShUpHeight.value - change < BOTUPHEIGHT:
                    change -= build_ele.BotShUpHeight.value - BOTUPHEIGHT
                    build_ele.BotShUpHeight.value = BOTUPHEIGHT
                else:
                    build_ele.BotShUpHeight.value -= change
                    change = 0.0
            if (change != 0) and (build_ele.BotShLowHeight.value > BOTLOWHEIGHT):
                if build_ele.BotShLowHeight.value - change < BOTLOWHEIGHT:
                    change -= build_ele.BotShLowHeight.value - BOTLOWHEIGHT
                    build_ele.BotShLowHeight.value = BOTLOWHEIGHT
                else:
            ...
            build_ele.RibHeight.value += change
        hole_height_from_top = value - build_ele.TopShHeight.value - RADIUS
        ...
        if name == "BotShUpHeight":
            ...
            hole_height_from_bot = value + build_ele.BotShLowHeight.value + RADIUS
        ...
        if name == "BotShLowHeight":
            ...
            hole_height_from_bot = build_ele.BotShUpHeight.value + value + RADIUS
            if hole_height_from_bot > build_ele.HoleHeight.value:
                build_ele.HoleHeight.value = hole_height_from_bot
        if name == "HoleHeight":
        ...
        if (name == "HoleDepth") and (value >= build_ele.BeamLength.value / 2.0):
            build_ele.HoleDepth.value = build_ele.BeamLength.value / 2.0 - RADIUS
        ...
        if name == "VaryingRibThick":
            varying_ribthick = build_ele.RibThick.value - DIFF
